...
if __name__ == '__main__':
    ...
    img = np.ones(shape=(400, 400, 3))

    gt_box = np.array([[100, 100, 100, 210, 210, 210, 210, 100, 100, 150, 1]])

    rotated_img, gtboxes_and_label = rotate_img_np(img, gt_box, 15)

    cv2.polylines(img, [np.reshape(gt_box[:, :-1], [-1, 2])], thickness=3, color=(0, 255, 0), isClosed=False)
    cv2.polylines(rotated_img, [np.reshape(gtboxes_and_label[:, :-1], [-1, 2])], thickness=3, color=(255, 0, 0), isClosed=False)
    print(rotated_img.shape)
    cv2.imwrite('demo.jpg', img)
    cv2.imwrite('demo_.jpg', rotated_img)
